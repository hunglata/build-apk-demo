...
object LudoSoundSynth {
    ...
    fun updateVolumes(mVol: Float, sVol: Float) {
        musicVolume = mVol
        sfxVolume = sVol
        try {
            mediaPlayer?.let {
                val v = (mVol * 0.4f).coerceIn(0f, 1f)
                it.setVolume(v, v)
            }
        } catch (e: Exception) {}
    }
    ...
    private fun playTone(
        ...
        val finalAmplitude = amplitude * sfxVolume
        scope.launch {
            try {
                ...
                for (i in 0 until numSamples) {
                    ...
                    buffer[i] = (sampleVal * finalAmplitude * Short.MAX_VALUE).toInt().toShort()
                }
    ...
    fun playSuperPawnHop() {
        scope.launch {
            if (!isSfxEnabled) return@launch
            // Special triumphant spring pop with high tone chime!
            playTone(220f, 880f, 160, 0.45f, "triangle")
            delay(100)
            playTone(880f, 1100f, 220, 0.45f, "sine")
        }
    }

    fun playSpecialFinalLanding() {
        scope.launch {
            if (!isSfxEnabled) return@launch
            // Heavy impact thud with a sweet celebration chime sparkle
            playTone(180f, 60f, 140, 0.5f, "square")
            delay(50)
            playTone(784f, 1174f, 180, 0.45f, "triangle")
        }
    }
